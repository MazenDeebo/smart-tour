// 25.11.2_webgl-1327-g527db53a6e
 ...
 * or waited on for a specific condition to be met.
 * @param <DataT> The type of the data being observed.
 */
export interface IObservable<DataT> {
	 ...
	 * @returns {Promise<DataT>} A promise that is resolved when `condition` returns true.
	 */
...
export interface IObservableMap<ItemT> {
	 ...
	 */
	waitUntil(condition: ICondition<Dictionary<ItemT>> | ConditionCallback<Dictionary<ItemT>>): Promise<Dictionary<ItemT>>;
}
/**
...
export declare namespace App {
	/**
	* `version` - allows segmenting the data by version.
	* `records` - accepts an object of records to be reported.
	* `alwaysTrack` - will disable the default filter that ignores subsequent tracking of the same fact.
...
export interface App {
   ...
   */
	 ...
	 * Track a fact.
	 *
	 * @param eventType the name of the fact. This name should be uniquely defined for each fact.
	 * @param options an object that optionally supports a version and an object of records to be reported.
	 *
	 * ```
	 * mpSdk.App.track('overlay-click',
	 * {
	 *    version: 1,
	 *    records: {
	 *      numberOfTimes: 2,
	 *      overlayId: 'root-overlay',
	 *    },
	 *    alwaysTrack: false,
	 * })
	 * ```
...
export interface Camera {
	 ...
	 * @internal
	 *
	 * Rotates the camera between two orientations
	 *
	 * @param startOrientation Orientation of the camera at the beginning of the transition.
	 * @param endOrientation Orientation of the camera at the end of the transition.
	 * @param duration How long the rotaton should last, in milliseconds.
	 * @param fadeToBlack If true, the screen will fade to black, followed by the camera instantly transitioning to the end rotation,
	 *                    followed by fading the scene in again.
	 * @param allowOverlappingTransitions If true, do not register this rotation transition as an external transition with the camera
	 *                                    module. This means that multiple simultaneous camera transitions are possible, so be very
	 *                                    carefule with this parameter. Defaults to false.
	 */
	rotateBetweenOrientations(startOrientation: Quaternion, endOrientation: Quaternion, duration: number, fadeToBlack: boolean, allowOverlappingTransitions: boolean): Promise<void>;
	/**
	 * @hidden
	 * @internal
	 *
	 * Stop any active camera operation
	 */
	stop(): Promise<void>;
	/**
...
export declare namespace Conversion {
	namespace TransformType {
		enum Coordinates {
			TO_API = "coordinatesToAPI",
			TO_SDK = "coordinatesToSDK"
		}
		enum SweepRotation {
			TO_API = "sweepRotationToAPI",
			TO_SDK = "sweepRotationToSDK"
		}
		enum CameraRotation {
			TO_API = "cameraRotationToAPI",
			TO_SDK = "cameraRotationToSDK"
		}
	}
}
export interface Conversion {
	TransformType: typeof Conversion.TransformType;
	/**
	 * Converts a 3D position or rotation between API and SDK orientations
	 *
	 *  @returns An object containing the conversion functions for each operator
	 *
	 * ```ts
	 ...
	 *
	 * ```ts
	 * const showcase = document.getElementById('showcaseIframe');
	 * const showcaseSize = {
	 *  w: showcase.clientWidth,
	 *  h: showcase.clientHeight,
	 * };
	 * const cameraPose; // get pose using: mpSdk.Camera.pose.subscribe
	 * const mattertag; // get a mattertag from the collection using: mpSdk.Mattertag.getData
	 *
	 * const screenCoordinate = mpSdk.Conversion.worldToScreen(mattertag.anchorPosition, cameraPose, showcaseSize)
	 * ```
	 */
...
export declare namespace Room {
	...
	namespace Bounds {
		/**
		 * @param on?: boolean - Whether or not the room dimensions are always visible
		 */
		function forceRoomDimensionVisibility(on?: boolean): Promise<void>;
	}
}
...
export declare namespace Tag {
	...
	type TagData = {
		id: string;
		/**
		 * Whether this tag is visible and accepts input.
		 * Modifying this value will toggle the tag's visibility and input handlers, and notify any existing observables.
		 */
		enabled: boolean;
		anchorPosition: Vector3;
...
export declare namespace View {
	...
	interface View extends IObservable<View> {
		 ...
		 * ```
		 */
		get layers(): IterableIterator<Layer>;
		/**
	...
	interface Layer extends IObservable<Layer> {
		 ...
		 *
		 * @param active Whether this Layer should be toggled on or off. If `active` is undefined, the state is flipped.
		 */
...
export interface View {
	ViewType: typeof View.ViewType;
	LayerType: typeof View.LayerType;
	/**
